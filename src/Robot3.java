...
public class Robot3 {
    ...
    private Robot3(String name, int level) {
        ...
        if ((name.isEmpty() //если имя которое задаёт пользователь будет пустым
        ...
        {
            this.name =
                    String.format("DefaultName_%d", defaultIndex++); //то будем использовать дефол найм и дефолтный индекс
        } else { // если ни одно условие не выполнилось, то мы возьмём имя, которое задал пользователь
            this.name = name;
        }

        Robot3.names.add(this.name); //добавляем в список
    ...
    public void power() {
        if (this.state == State.Off) { // если система выключена
            this.powerOn(); //вызвать логику включения
            this.state = State.On; //поменять состояние на то что робот включен
        } else {
            this.powerOff();
            this.state = State.Off;
        }
